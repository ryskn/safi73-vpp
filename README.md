...
**Priority 順の再検証** (§2.12): `-revalidate <interval>` を指定すると、全 SR Policy を
定期的に再検証する。処理順は priority (Priority sub-TLV、**低い値ほど高優先**、既定 128。
policy 全体では CP 群の最小値) に従い、重要な policy から先に処理する。`-verify-sids` と
併用すると、BGP イベントが無くても FIB の変化 (first SID の経路消失/復活) で policy を
down/up できる。再同期 (gobgp 再接続) 直後にも 1 回実行する。

**Drop-upon-invalid** (§8.2, I-Flag): BSID sub-TLV の I-Flag 付き CP が active だった policy が
invalid になったとき、既定の「削除して IGP フォールバック」(fail-open) ではなく、endpoint 宛の
steering を **drop 経路に差し替えて落とす** (fail-closed)。policy が valid に戻れば drop を解除して
再投入し、policy 自体が消滅 (全 CP withdraw) したら drop も撤去する。endpoint steering が
使えない構成 (`-steer=false` / null endpoint) では発動できず、警告して既定動作にフォールバック。
steering→drop の入れ替えの一瞬だけ既定経路へ逃げる余地がある点は既知の制限。

**SID 到達性検証** (§5.1): `-verify-sids` を指定すると、各 SID-list の first SID と
V-Flag (SID verification 要求) 付き SID を VPP FIB (`ip_route_lookup`, `-fib` のテーブル) で
LPM 解決し、drop 以外の path に解決できない SID-list を invalid にする。照会自体の失敗は
fail-open (到達可能扱い + 警告)。uSID 圧縮時は verification 要求を carrier 先頭 SID に引き継ぐ。
既定 off — 有効にする場合は first SID への経路 (IGP 等) が VPP に入っていることが前提。

**BSID 動的割当** (§6.2.1): `-bsid-pool <prefix>` を指定すると、BSID 未指定の CP には
プールから BSID を動的に束縛する。割当は policy 単位で、active CP が切り替わっても維持される。
S-Flag (Specified-BSID-only, §6.2.3) 付きで BSID 未指定の CP は invalid。プール未指定時は
BSID 無し CP を候補外にする (従来動作)。注意: 割当は daemon のメモリ上のみで、再起動すると
別の BSID になりうる (旧 BSID の残骸は `-orphan-gc` で回収)。
...
| `-bsid-pool` | (無効) | BSID 未指定の CP への動的 BSID 割当プール (RFC 9256 §6.2.1) |
| `-verify-sids` | false | first SID / V-Flag 付き SID の FIB 解決チェック (RFC 9256 §5.1) |
| `-revalidate` | 0 (無効) | 全 policy を priority 順に再検証する間隔 (RFC 9256 §2.12) |
